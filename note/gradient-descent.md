...
We subtract the slope because the slope (gradient) mathematically always points *up* the steepest part of the curve. Since we want to go *down* to the minimum, we take a step in the opposite direction.
...
3. The next guess is calculated by: 
    $$x_{1\text{, next}} = x_{1\text{, current}} - \text{slope}_1 \cdot \text{lr} \\
    x_{2\text{, next}} = x_{2\text{, current}} - \text{slope}_2 \cdot \text{lr} \\
    \vdots \\
    x_{n\text{, next}} = x_{n\text{, current}} - \text{slope}_n \cdot \text{lr}$$
...
What is it about the learning rate? Without the learning rate, $x_\text{next} = x_\text{current} - \text{slope}$, the step size equals the raw slope. This is bad because the raw slope value is usually way too large, causing the algorithm to aggressively jump past the minimum and explode outward (divergence). By applying a learning rate, typically a value between 0 and 1, we scale the step size down. This prevents overshooting and smoothly shrinks the steps as the slope naturally flattens near the target.
...
For a simple linear regression $\bar{y}=mx+b$, we aim to find the best fit $m$ and $b$. Which means, techniqually speaking, they are our variables. So, our function on which we perfom gradient descent is not $\bar{y}$, but the mean squared error (MSE) rather than simply the sum of squared errors (SSE), in order to keep the gradient manageable and learning rate stable regardless of the dataset size:
    $$MSE = f(m, b) = \frac{1}{n} \sum_{i=1}^n(y_i - \bar{y}_i)^2 = \frac{1}{n} \sum_{i=1}^n(y_i - mx_i - b)^2$$

The partial slopes with respect to $m$ and $b$ are:
    $$\frac{\partial f}{\partial b} = -\frac{2}{n}\sum(y - mx - b) \qquad
    \frac{\partial f}{\partial m} = -\frac{2}{n}\sum(x)(y - mx - b)$$
...
We do this until our $c$ values $c_b=\frac{\partial f}{\partial b}\cdot lr$ and $c_m=\frac{\partial f}{\partial m}\cdot lr$ are so close to zero. In the end, we take the final next guesses for both $b$ and $m$ as the best fit parameters. Because, remember that the goal of linear regression is to find the input values for which the mean squared error (loss function) is zero slope.
...
One thing we do need to worry though is about the range (scale) of the input data. For a dataset containing age and salary, for example, the age feature is usually in the range between 1-100, while salary is between say 1,000-100,000. We use the same learning rate for all parameters of the features. And that's where we find a critical issue. The learning rate may converge on the age feature but may diverge for the salary. We can find different learning rates tailored to each features, or we can standardize (scale down to a specific range) the entire dataset and use one learning rate for all.
